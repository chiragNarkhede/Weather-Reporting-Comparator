...
public class UiOps {
	...
	public static boolean checkTemp(String uiTemp, WeatherReportResponse responseObj) throws MatcherException {
		...
		if (calculatedRange >= 0 && calculatedRange < range) {
			result = true;
			log.info("Compare temperature from UI and API using variance logic.Result:" + result);
			System.out.println("Compare temperature from UI and API using variance logic.Result:" + result);
		} else {
